...
std::vector<unsigned char> TextureHelper::load_png(const std::filesystem::path &_path,
                                                   unsigned &_w,
                                                   unsigned &_h,
                                                   ENCODING _encoding) {
  LodePNGColorType _colorType = LodePNGColorType::LCT_RGBA;
  switch (_encoding) {
    case ENCODING::RGBA_8: break;
    case ENCODING::RGB_8: _colorType = LodePNGColorType::LCT_RGB;
      break;
    case ENCODING::R_8: _colorType = LodePNGColorType::LCT_GREY;
      break;
    default:
...
std::string encoding_to_string(TextureHelper::ENCODING _encoding) {
  using ENCODING = TextureHelper::ENCODING;
  switch (_encoding) {
    case ENCODING::R_8: return "R_8";
    case ENCODING::RGB_8: return "RGB_8";
    case ENCODING::RGBA_8: return "RGBA_8";
    case ENCODING::INVALID: return "";
    default: abort(); // hard error because this simply should never happen, ill write a test to make sure of it
  };
};
...
Texture::Texture(ENCODING _encoding, size_t _width, size_t _height, const allocator_type &_alloc) :
    ...
    width_{_width},
    height_{_height},
    data_{(_width * _height) * (size_t) ((uint8_t) _encoding), _alloc},
    alloc_{_alloc} {};
...
Texture::Texture(ENCODING _encoding, const std::filesystem::path &_pngPath, const allocator_type &_alloc) :
  ...
  auto _pxData = this->load_png(_pngPath, _w, _h, _encoding);
  this->data_.assign(_pxData.begin(), _pxData.end());
